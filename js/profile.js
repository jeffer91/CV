(() => {
  ...
  const evidenceUrl=x=>x?.evidence||x?.url||"";
  function list(items,renderer,empty="Sin información publicada."){return items?.length?items.map(renderer).join(""):`<p class="muted">${esc(empty)}</p>`}
  function description(x){return x._customText||x.description||""}
  ...
  function render(){
    ...
    const exp=CVStore.relevant(data.experience,profile,7);
    ...
    const education=CVStore.rankAll(data.education,profile,6);
    ...
    $("contactActions").innerHTML=[
      ...
      data.profile.linkedin&&`<a class="button ghost" target="_blank" rel="noopener" href="${esc(data.profile.linkedin)}">LinkedIn</a>`,
    ...
    $("experienceList").innerHTML=list(exp,x=>`
      <article class="timeline-item"><div class="timeline-marker"></div><div class="timeline-card">
        ...
        ${description(x)?`<p>${esc(description(x))}</p>`:""}
        ${x.bullets?.length?`<ul>${x.bullets.slice(0,4).map(b=>`<li>${esc(b)}</li>`).join("")}</ul>`:""}
        ${button(evidenceUrl(x),"Ver evidencia")}
      </div></article>`);
    ...
    $("teachingList").innerHTML=list(teach,x=>`
      <article class="info-card featured-card"><div class="item-top"><div><h3>${esc(x.role)}</h3><p class="muted strong">${esc(x.organization)}</p></div><span class="pill">${esc(x.period)}</span></div>
        ...
        ${button(evidenceUrl(x),"Ver certificado")}
      </article>`);
    ...
    $("educationGrid").innerHTML=list(education,x=>`
      ...
      ${x.status?`<p class="muted">${esc(x.status)}</p>`:""}${x._customText?`<p>${esc(x._customText)}</p>`:""}${button(evidenceUrl(x),"Ver título / respaldo")}</article>`);

    $("skillsGrid").innerHTML=list(skills,x=>`
      <article class="info-card"><h3>${esc(x.category)}</h3>${x._customText?`<p>${esc(x._customText)}</p>`:""}<div class="cv-tags">${(x.items||[]).map(s=>`<span class="cv-tag">${esc(typeof s==="string"?s:s.name)}</span>`).join("")}</div></article>`);

    $("coursesGrid").innerHTML=list(courses,x=>`
      <article class="info-card compact-card"><p class="eyebrow">${esc(x.category)}</p><h3>${esc(x.title)}</h3><p>${esc(x.issuer)}</p>${x._customText?`<p>${esc(x._customText)}</p>`:""}${button(evidenceUrl(x),"Certificado")}</article>`);

    $("projectsGrid").innerHTML=list(projects,x=>`
      <article class="info-card"><p class="eyebrow">Proyecto</p><h3>${esc(x.title)}</h3><p>${esc(x._customText||x.description)}</p>${button(evidenceUrl(x),"Evidencia")}</article>`);

    $("publicationsGrid").innerHTML=list(pubs,x=>`
      <article class="info-card"><p class="eyebrow">${esc(x.type)}</p><h3>${esc(x.title)}</h3><p>${esc(x._customText||x.description)}</p>${button(evidenceUrl(x),"Abrir")}</article>`);
