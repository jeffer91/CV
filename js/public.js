(() => {
  const base = structuredClone(window.CV_DATA);
  const saved = safeParse(localStorage.getItem('cv_public_override'));
  const data = saved ? mergeData(base, saved) : base;
  const $ = (id) => document.getElementById(id);
  const esc = (s='') => String(s).replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));

  function safeParse(raw){ try { return raw ? JSON.parse(raw) : null; } catch { return null; } }
  function mergeData(target, patch){ if (patch?.profile) target.profile = {...target.profile, ...patch.profile}; return target; }
  ...
  async function loadPublishedEvidence(){
    ...
    if(!cfg.url || !cfg.anon || !window.supabase?.createClient) return;
    try{
      const client=window.supabase.createClient(cfg.url,cfg.anon);
      const {data:links,error}=await client.from('evidence_links').select('entity_type,entity_id,public_url');
      if(error || !links) return;
      links.forEach(link=>{
        if(link.entity_type==='education'){
          const item=data.education.find(x=>x.id===link.entity_id); if(item)item.evidence=link.public_url;
        }else if(link.entity_type==='course'){
          const item=data.courses.find(x=>x.id===link.entity_id); if(item)item.url=link.public_url;
        }
      });
      renderEvidenceSections();
    }catch(_){ /* El perfil sigue funcionando aunque Supabase no responda. */ }
  }
